...
import { Vendor, UpdateVendorRequest } from '@/types/vendor'
...
export const EditVendorModal = ({ isOpen, vendor, onClose, onUpdate, onDelete, isLoading = false }: EditVendorModalProps) => {
  ...
  const changedFields = useMemo(() => {
    ...
    const changes: Partial<UpdateVendorRequest> = {}
    ...
    Object.entries(fieldMap).forEach(([formKey, vendorKey]) => {
      if (formData[formKey as keyof UpdateVendorRequest] !== vendor[vendorKey]) {
        changes[formKey as keyof UpdateVendorRequest] = formData[formKey as keyof UpdateVendorRequest]
      }
    })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onUpdate(formData as UpdateVendorRequest)
      onClose()
    } catch (error) {
  ...
  const handleInputChange = (field: keyof UpdateVendorRequest, value: any) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value
    }))
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {showDeleteConfirm && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              <p className="text-red-800 font-medium mb-3">Are you sure you want to delete this vendor?</p>
